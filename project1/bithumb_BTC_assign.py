...
while(1):
    ...
    if(not os.path.isfile("book-%s-bithumb.csv"%(req_date))):
        df.to_csv("book-%s-bithumb.csv"%(req_date), index=False, mode = 'a', sep='|')
    df.to_csv("book-%s-bithumb.csv"%(req_date), index=False, header=False, mode = 'a', sep='|')
